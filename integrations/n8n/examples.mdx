...
2. **IF Node**: Filter for specific tags
   ```javascript
   {{ $json.payload.tags && $json.payload.tags.includes('new-subscriber') }}
   ```
...
1. **Falconyte Trigger**: Subscribe to `email.contact.saved`
2. **Clearbit API**: Get company and role data
3. **Hunter.io API**: Verify email deliverability
4. **FullContact API**: Get social media profiles
5. **Function Node**: Merge all enrichment data
   ```javascript
   return {
     email: $json.payload.email,
     custom_fields: {
   ...
   ```
6. **Falconyte Action**: Update contact with enriched data
7. **Conditional Logic**: Route based on enrichment results
